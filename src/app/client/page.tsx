    ...
    <>
      ...
      <div className="hidden md:flex flex-col w-full min-h-screen relative z-10 overflow-hidden bg-[var(--bg-color)]">
        <div className="max-w-[1400px] mx-auto w-full px-12 pt-20 pb-32 flex flex-col items-start">
          ...
          {/* 1. Header - Exact match to settings style */}
          ...
          <nav className="w-full mb-12 flex items-center justify-start gap-10 overflow-x-auto hide-scrollbar pb-6 border-b border-[var(--border-color)]/10">
             <motion.button 
               ...
               className={`relative text-[11px] font-black uppercase tracking-[0.3em] transition-all duration-300
                 ${!selectedCategoryId 
                   ...
                   : 'text-[var(--text-secondary)] opacity-10 hover:opacity-60'
                 }`}
             >
               Все услуги
               {!selectedCategoryId && (
                 <motion.div 
                   ...
                   className="absolute -bottom-6 left-0 right-0 h-[2px] bg-[var(--text-primary)]"
            ...
            <AnimatePresence mode="wait">
              {filteredServices.length === 0 ? (
                ...
                <motion.div 
                  ...
                  className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 2xl:grid-cols-6 gap-x-4 gap-y-10"
                >
                   {filteredServices.map((service, index) => (
                      <motion.div 
                        ...
                        transition={{ 
                          duration: 0.5, 
                          delay: Math.min(index * 0.05, 0.3), // Cap delay for large lists
                          ease: [0.215, 0.61, 0.355, 1] 
                        }}
                        className="group flex flex-col cursor-pointer transition-all duration-300"
                        onClick={() => router.push(`/tarif?id=${service.id}`)}
                      >
                        {/* Image Card - Vertical WB Focus */}
      ...
      <div className="md:hidden min-h-screen bg-[var(--bg-color)] text-[var(--text-primary)] pb-32 relative overflow-hidden animate-page-fade-in transition-colors duration-300">
          ...
          <div className="flex items-center gap-2 h-[44px] w-full group">
            ...
            <button
              ...
              className={`w-[44px] h-[44px] rounded-full flex items-center justify-center transition-all shrink-0 active:scale-95 ${
                search.trim() ? 'bg-[var(--text-secondary)]' : 'bg-[var(--nav-bg)]'
              }`}
            >
            ...
            {categories.map((cat) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`text-[11px] font-bold text-center leading-tight uppercase tracking-[0.05em] transition-colors duration-300 ${isActive ? 'text-[#1AE8E8]' : 'text-[var(--text-primary)]'}`}>
                    {cat.name}
                  </span>
                </button>
              );
                ...
                {filteredServices.map((service) => (
                  <div key={service.id} className="flex flex-col group cursor-pointer" onClick={() => router.push(`/tarif?id=${service.id}`)}>
                    ...
                    <div className="w-full aspect-[153.5/159] rounded-[23.5px] overflow-hidden bg-[var(--card-bg)] mb-4 relative border border-[var(--border-color)] transition-colors duration-300">
                      <img src={service.coverImage} alt={service.name} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
