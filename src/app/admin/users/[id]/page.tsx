...
import { ChevronLeft, UserPlus, User, Star, Trash2 } from "lucide-react";
...
export default function UserDetailsPage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-color)] text-[var(--text-primary)] font-sans flex justify-center selection:bg-[#1AE8E8]/30 overflow-x-hidden w-full">
      {/* DESKTOP VIEW */}
            ...
            <div className="flex items-center gap-10">
              ...
              <div className="flex flex-col">
                ...
                <div className="flex items-center gap-2">
                   <span className="text-[15px] font-bold text-white/90">Пользователь #{formatPhone(user.phone)}</span>
                </div>
              </div>
            </div>
                ...
                <div className="max-w-[800px] mx-auto flex flex-col gap-24">
                   ...
                   <div className="flex flex-col gap-16">
                      ...
                      <div className="grid grid-cols-12 gap-12">
                         ...
                         <div className="col-span-12 xl:col-span-4">
                            ...
                            <div 
                              ...
                              className={`aspect-square rounded-[32px] border-2 border-dashed transition-all duration-700 cursor-pointer overflow-hidden group relative flex flex-col items-center justify-center w-full max-w-[280px] ${
                                user.avatar ? 'border-transparent' : 'border-[var(--border-color)] hover:border-[var(--accent-cyan)]/40 hover:bg-[var(--accent-cyan)]/[0.02]'
                              }`}
                            >
                              {user.avatar ? (
                                <>
                                  <img src={user.avatar} alt="Avatar Preview" className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105" />
                         ...
                         <div className="col-span-12 xl:col-span-8 flex flex-col justify-center gap-12 xl:pl-8">
                            {/* Phone Input Group (Readonly for Edit) */}
                            <div className="flex flex-col gap-3 group border-b border-[var(--border-color)] pb-4 opacity-70">
                               <span className="text-[10px] font-black uppercase tracking-[0.2em] text-white/20 transition-colors">Телефон (Нельзя изменить)</span>
                               <input 
                                 type="text"
                                 value={formatPhone(user.phone)}
                                 readOnly
                                 className="bg-transparent text-2xl font-bold text-white outline-none w-full cursor-not-allowed"
                               />
                            </div>
                            ...
                            {/* Partner Switch */}
                            <div className="flex items-center justify-between p-6 rounded-[24px] border border-[var(--border-color)] bg-white/[0.01] hover:bg-white/[0.02] cursor-pointer transition-colors" onClick={() => handleSave("isPartner", !user.isPartner)}>
                               <div className="flex flex-col gap-1">
                                  <span className={`text-sm font-bold tracking-tight transition-colors ${user.isPartner ? 'text-[var(--accent-cyan)]' : 'text-white'}`}>Статус партнера</span>
                               ...
                               <div className={`w-12 h-6 rounded-full relative transition-all duration-500 ${user.isPartner ? 'bg-[var(--accent-cyan)]' : 'bg-white/10'}`}>
                                  <div className={`w-4 h-4 rounded-full bg-white absolute top-1 transition-all duration-500 ${user.isPartner ? 'left-[26px]' : 'left-1'}`} />
                               </div>
                            </div>
                         </div>
                      </div>
                   ...
                   <div className="flex flex-col gap-10 mt-6">
                      <div className="flex items-end justify-between border-b border-[var(--border-color)] pb-8">
                         <div className="flex flex-col gap-2">
                            <h2 className="text-4xl font-black font-cera uppercase tracking-tighter text-white">Активные заказы</h2>
                         </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-6">
                         {activeOrders.length === 0 ? (
                           <div className="col-span-2 py-12 flex flex-col items-center justify-center border border-dashed border-[var(--border-color)] rounded-3xl opacity-50">
                              <span className="text-[13px] font-bold">Нет активных заказов</span>
                           </div>
                         ) : (
                           activeOrders.map(order => (
                              <div
                                key={order.id}
                                className="bg-white/[0.02] border border-[var(--border-color)] rounded-3xl p-6 flex flex-col gap-4 cursor-pointer hover:bg-white/[0.05] hover:border-[var(--accent-cyan)]/30 transition-all group"
                                onClick={() => router.push(`/admin/orders/${order.id}`)}
                              >
                                <div className="flex justify-between items-start">
                                   <span className="text-[18px] font-bold text-white mb-2 leading-tight line-clamp-1">{order.title}</span>
                                   <span className="text-[12px] font-black uppercase tracking-widest text-[#FF8C67] shrink-0">№{order.orderNumber || order.id.slice(-2)}</span>
                                </div>
                                <div className="flex items-center justify-between mt-auto">
                                   <span className="text-[11px] font-medium text-white/40">{order.tariff}</span>
                                   <span className="text-[14px] font-bold text-[var(--accent-cyan)]">{Number(order.price).toLocaleString()} ₽</span>
                                </div>
                              </div>
                           ))
                         )}
                   ...
                   <div className="flex justify-center pt-8">
                      <div className="w-full flex flex-col group/preview">
                          <div className="aspect-[3/4] bg-white/[0.02] rounded-[32px] border border-white/[0.05] relative overflow-hidden mb-6 shadow-2xl transition-all duration-700 hover:border-[var(--accent-cyan)]/20 hover:shadow-[0_20px_50px_rgba(26,232,232,0.1)]">
                              {user.avatar ? (
                                <img src={user.avatar} className="w-full h-full object-cover opacity-80" alt="" />
                              ) : (
                              ...
                              <div className="absolute top-6 right-6 text-white/40 group-hover/preview:text-[var(--accent-cyan)] transition-colors">
                                 {user.isPartner && <Star className="w-6 h-6 fill-current text-[var(--accent-cyan)] drop-shadow-[0_0_10px_rgba(26,232,232,0.5)]" />}
                              </div>

                              <div className="absolute bottom-6 left-6 right-6 flex flex-col gap-2">
                                 <h3 className="text-2xl font-bold font-cera text-white leading-tight drop-shadow-md">
                                    {editedName || user.name || "Профиль"}
                                 </h3>
                                 <div className="text-[12px] font-bold text-[var(--accent-cyan)] opacity-80 tracking-widest">{formatPhone(user.phone)}</div>
                                 {editedEmail && (
                                   <div className="text-[10px] font-medium text-white/50">{editedEmail}</div>
                                 )}
                              </div>
                          </div>
                      </div>
      ...
      <div className="md:hidden w-full relative min-h-screen bg-transparent flex flex-col items-center">
        ...
        <header className="fixed top-0 w-full max-w-[375px] h-[102px] z-[100] bg-black/40 backdrop-blur-3xl border-b border-white/5 shadow-2xl transition-colors duration-300">
          <svg width="375" height="102" viewBox="0 0 375 102" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full absolute -z-10 left-0 bottom-0 pointer-events-none">
            ...
            <path d="M375 102V101H0V102V103H375V102Z" fill="transparent" mask="url(#path-1-inside-1_1706_5018)"/>
        ...
        <main className="flex-1 w-full pt-[118px] md:pt-10 px-6 pb-20 space-y-8 md:grid md:grid-cols-[1fr_2fr] md:gap-10 md:space-y-0">
          ...
          <div className="space-y-5">
             <div className="space-y-2">
                ...
                <div className="w-full h-[60px] bg-white/5 backdrop-blur-sm rounded-[20px] flex items-center px-5 border border-white/10 shadow-sm">
                  <input 
                    ...
                    onChange={e => setEditedName(e.target.value)}
                    onBlur={() => handleSave("name", editedName)}
                    className="w-full bg-transparent border-none outline-none text-[16px] font-bold text-[var(--text-primary)] transition-colors focus:text-[#1AE8E8]"
             ...
             <div className="space-y-2">
                ...
                <div className="w-full h-[60px] bg-white/5 backdrop-blur-sm rounded-[20px] flex items-center px-5 opacity-80 border border-white/10 shadow-sm">
                  <input 
                    value={formatPhone(user.phone)}
                    readOnly
             ...
             <div className="space-y-2">
               ...
               <div className="bg-white/5 backdrop-blur-sm rounded-[20px] p-5 flex items-center justify-between active:scale-[0.98] transition-all cursor-pointer border border-white/10 shadow-sm hover:shadow-lg" onClick={() => handleSave("isPartner", !user.isPartner)}>
                  <div className="space-y-0.5" style={{ fontFamily: 'var(--font-cera)' }}>
                     <p className={`text-[16px] font-bold tracking-tight transition-colors ${user.isPartner ? 'text-[#1AE8E8]' : 'text-[var(--text-primary)]'}`} style={{ lineHeight: '1.2' }}>Статус партнера</p>
                     <p className="text-[11px] font-bold text-[var(--text-secondary)]" style={{ lineHeight: '1.2' }}>Доступ к партнерской панели</p>
                  </div>
                  <div 
                    className={`w-[58px] h-[32px] rounded-full relative transition-all duration-500 overflow-hidden border ${user.isPartner ? 'bg-[#1AE8E8] border-[#1AE8E8]' : 'bg-[var(--bg-color)] border-[var(--border-color)]'}`}
                  >
                     <div 
                       className={`absolute top-1/2 -translate-y-1/2 w-6 h-6 rounded-full transition-all duration-500 ease-[cubic-bezier(0.68,-0.55,0.27,1.55)] ${user.isPartner ? 'right-1 bg-white shadow-lg' : 'left-1 bg-[var(--border-color)]'}`}
                     />
                  </div>
               </div>
             </div>
          ...
          <div>
              <h2 className="text-[22px] md:text-[32px] font-bold text-[var(--text-primary)] pt-2 md:pt-0 mb-6">Активные заказы</h2>

              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                 {activeOrders.length === 0 ? (
                   <div className="py-10 md:py-32 col-span-full text-center opacity-30 bg-[var(--card-bg)] rounded-[32px] border border-[var(--border-color)]">
                   ...
                   activeOrders.map(order => (
                     <div 
                       key={order.id}
                       onClick={() => router.push(`/admin/orders/${order.id}`)}
                       className="bg-black/40 backdrop-blur-md rounded-[32px] p-6 flex flex-col justify-between active:scale-[0.98] transition-all cursor-pointer border border-white/5 shadow-xl hover:shadow-2xl group min-h-[300px]"
                     >
                        <div className="space-y-4">
                            <div className="flex justify-between items-start">
                               <h3 className="text-[22px] font-bold leading-[120%] max-w-[210px] group-hover:text-[#1AE8E8] transition-colors">{order.title}</h3>
                        ...
                        <div className="flex items-center justify-between pt-auto mt-4 shrink-0">
                           <div className="flex gap-[4px]">
                              {[1, 2, 3, 4, 5, 6].map((step) => (
                                <div 
                                  key={step} 
                                  className={`h-[10px] w-[20px] sm:w-[30px] rounded-full transition-all duration-500 ${step <= (order.status === 'completed' ? 6 : order.status === 'pending' ? 1 : 4) ? 'bg-[var(--text-primary)] [html.day-theme_&]:bg-[#141414]' : 'bg-[var(--border-color)]'}`}
                                />
                              ))}
                           </div>
