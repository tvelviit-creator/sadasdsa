...
export default function AdminTemaPage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-color)] text-[var(--text-primary)] font-sans selection:bg-white/10 transition-colors duration-300 flex justify-center">
      {/* 375px Design Container */}
      <div className="relative w-full max-w-[375px] min-h-screen bg-[var(--bg-color)] transition-colors duration-300">
        ...
        <div className="absolute top-0 left-0 w-[80px] h-[102px]">
          <button onClick={() => router.back()} className="w-full h-full appearance-none cursor-pointer focus:outline-none" aria-label="Назад" />
        </div>

        {/* Theme Toggles */}
        <div className="absolute top-[245px] left-0 w-full h-[110px] flex justify-between px-[24px] gap-[10px]">
          <button onClick={() => changeTheme('auto')} className="flex-1 h-full appearance-none cursor-pointer focus:outline-none" aria-label="Авто тема" />
          <button onClick={() => changeTheme('day')} className="flex-1 h-full appearance-none cursor-pointer focus:outline-none" aria-label="Дневная тема" />
          <button onClick={() => changeTheme('night')} className="flex-1 h-full appearance-none cursor-pointer focus:outline-none" aria-label="Ночная тема" />
        </div>
