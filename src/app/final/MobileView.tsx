...
const CustomCheckIcon = ({ color = "var(--text-primary)" }: { color?: string }) => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="transition-colors duration-300 shrink-0">
...
interface FinalMobileViewProps {
  router: any;
...
export default function MobileView({
  ...
  executorName,
  executorAvatar
}: FinalMobileViewProps) {
  return (
    <div className="min-h-screen bg-[var(--bg-color)] text-[var(--text-primary)] flex flex-col pb-40 transition-colors duration-300 md:hidden overflow-x-hidden relative mx-auto w-full max-w-[375px]">
      
      {/* 1. Progress Bar from Backup */}
      ...
      <header className="px-6 flex items-center justify-between mb-8 relative">
        ...
        <h1 className="text-[20px] font-bold absolute left-1/2 -translate-x-1/2 tracking-tight whitespace-nowrap">
          {selectedPackages.serviceName}
        </h1>
        <div className="w-10" />
      </header>

      {/* Content List from Backup Style */}
      <div className="px-6 space-y-8">
        ...
        <div className="border-b border-[var(--border-color)] pb-6 transition-colors duration-300">
          ...
          <h2 className="text-[24px] font-normal text-[var(--text-primary)] transition-colors duration-300 leading-tight">{selectedPackages.serviceName}</h2>
        </div>
        ...
        <div>
          ...
          <h2 className="text-[28px] font-normal text-[var(--text-primary)] mb-2 transition-colors duration-300 leading-tight">{selectedPackages.tariffName}</h2>
          ...
          <div className="space-y-4">
            {selectedPackages.tariffFeatures?.map((f: any, i: number) => (
        ...
        <div>
          ...
          <h2 className="text-[28px] font-normal text-[var(--text-primary)] mb-2 transition-colors duration-300 leading-tight">{selectedPackages.designName}</h2>
          <p className="text-[var(--text-secondary)] text-[14px] transition-colors duration-300 leading-relaxed">
        ...
        <div>
          ...
          <p className="text-[36px] font-bold text-[var(--text-primary)] transition-colors duration-300 tracking-tight">{totalPrice.toLocaleString()} ₽</p>
        </div>

        {/* Agreement Radio/Checkbox */}
        <div
          onClick={() => setIsAgreed(!isAgreed)}
          className="flex items-start gap-4 cursor-pointer pt-2 pb-10"
        >
          <div className={`
             w-6 h-6 rounded-full border-[2px] mt-1 flex items-center justify-center shrink-0 transition-all duration-300
      ...
      {/* Floating Pay Button from Backup */}
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[375px] bg-gradient-to-t from-[var(--bg-color)] via-[var(--bg-color)] to-transparent pt-10 pb-8 z-50 px-6 flex justify-center pointer-events-none">
        <button
          onClick={() => router.push("/pay")}
          disabled={!isAgreed}
          className={`
            w-full max-w-[327px] h-[56px] rounded-full font-bold text-[16px] transition-all flex items-center justify-center tracking-tight pointer-events-auto shadow-xl
            ${isAgreed
